...
public class MemberDAO {
	...
	public MemberDTO getMemberInfo(String id) {
		...
		try {
			...
			sql = "select Mem_id,Mem_pw, Mem_email, Mem_birth, Mem_phone, Mem_postcode, Mem_addr1, Mem_addr2, Mem_addr3, Mem_mType, Mem_grade from member "
					+ " where Mem_id=?";
			pstmt = con.prepareStatement(sql);
	 ...
	 * int result = -1;
	 * 
	 * try { // DB 연결 con = getCon();
	 * 
	 * sql = "select Mem_pw from member where Mem_id=?"; // 비회원이다. PreparedStatement
	 * pstmt = con.prepareStatement(sql);
	 * 
	 * pstmt.setString(1, dto.getMem_id());
	 * 
	 ...
	 * // 5. 데이터처리 if(rs.next()) {
	 * 
	 * if(dto.getMem_pw().equals(rs.getString("Mem_pw"))) { //본인 result = 1; }else {
	 * // 비밀번호 오류 result = 0; } }else { // 비회원(아이디 정보없음) result = -1; }
	 * 
	...
	public void naverJoin(MemberDTO dto) {
		...
		try {
			...
			sql = "insert into member(Mem_num,Mem_name,Mem_id,Mem_pw,Mem_phone,Mem_email) " + "values(?,?,?,?,?,?)";
			pstmt = con.prepareStatement(sql);
	...
	public MemberDTO memberInfo(String Mem_id) {
		...
		try {
			...
			sql = "select Mem_num,Mem_name,Mem_grade,rCount from member " + " where Mem_id=?";
	...
	public MemberDTO getGradeInfo(String Mem_id) {
		...
		try {
			...
			sql = "select Mem_name,rCount from member " + " where Mem_id=?";
	...
	// getTicketCount()
	public int getTicketCount(int mem_num) {
		int cnt = 0;
		try {
			con = getCon();
	...
	// updateGrade
	public void updateGrade(int TicketCount, int mem_num) {

		try {
			con = getCon();

			sql = "update member set Mem_grade=?, rCount=? where Mem_num=?";
			pstmt = con.prepareStatement(sql);
			if (TicketCount < 10) {
				pstmt.setString(1, "Bronze");
				System.out.println("브론즈");

			} else if (TicketCount >= 10 && TicketCount < 20) {
